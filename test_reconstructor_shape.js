// Test script for reconstructor with shape-based positions
const nearley = require("nearley");
const grammar = require("./src/parser/parser.js");

async function runTests() {
    const { default: reconstructor } = await import("./src/parser/reconstructor.mjs");

    function testReconstruct(input, testName) {
        console.log(`\n=== ${testName} ===`);
        
        const parser = new nearley.Parser(nearley.Grammar.fromCompiled(grammar));
        try {
            parser.feed(input);
            if (parser.results.length > 0) {
                const parsedDSL = parser.results[0];
                console.log("✅ Parsed successfully");
                
                try {
                    const reconstructed = reconstructor(parsedDSL);
                    console.log("✅ Reconstructed successfully");
                    console.log("Reconstructed DSL:");
                    console.log(reconstructed);
                    return reconstructed;
                } catch (reconstructError) {
                    console.log("❌ Reconstruct error:", reconstructError.message);
                    return null;
                }
            } else {
                console.log("❌ No parse results");
                return null;
            }
        } catch (error) {
            console.log("❌ Parse error:", error.message);
            return null;
        }
    }

    console.log("Testing reconstructor with shape-based positions...");

    // Test 1: Regular position
    const test1 = `array arr1 = {
	value: [1, 2, 3]
}

page 3x3
show arr1 (0, 1)`;
    testReconstruct(test1, "Regular position");

    // Test 2: Horizontal range
    const test2 = `array arr1 = {
	value: [1, 2, 3]
}

page 3x3
show arr1 (0..1, 0)`;
    testReconstruct(test2, "Horizontal range");

    // Test 3: Rectangle range
    const test3 = `array arr1 = {
	value: [1, 2, 3]
}

page 3x3
show arr1 (0..1, 0..1)`;
    testReconstruct(test3, "Rectangle range");
}

runTests().catch(console.error);
