...
var drawTextDiagram = /* @__PURE__ */ __name((svg, textDiagram, yOffset) => {
  ...
  textDiagram.elements.forEach((element, index) => {
    drawElement4(group, element, index);
  });
  if (textDiagram.label) {
    const totalHeight = textDiagram.elements.length * 40;
    const labelYPosition = totalHeight + 20;
...
var drawElement4 = /* @__PURE__ */ __name((svg, element, positionIndex) => {
  const group = svg.append("g");
  const elementX = 50;
  const elementY = positionIndex * 40;
  const lines = element.split("\n");
  lines.forEach((line, lineIndex) => {
    group.append("text").attr("x", elementX).attr("y", elementY + lineIndex * 20).attr("fill", "black").attr("font-size", "18").attr("dominant-baseline", "hanging").attr("class", "textElement").text(line);
  });
}, "drawElement");
