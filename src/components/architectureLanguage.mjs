...
export function registerCustomLanguage(monaco) {
  ...
  function getLabelOrientationState(valueText) {
  ...
  const ANNOTATION_SHIFT_KEYS = ANNOTATION_SIDES.flatMap((side) =>
    SHIFT_SIDES.map((shift) => `${side}.shift.${shift}`),
  );

  const ANNOTATION_ALL_KEYS = [
    ...ANNOTATION_BASE_KEYS,
    ...ANNOTATION_SHIFT_KEYS,
  ];

  const ANNOTATION_PROP_KEYS = ANNOTATION_ALL_KEYS.map(
    (k) => `annotation.${k}`,
  );

  const annotationPropRegex =
  ...
  const annotationNonSidePropRegex =
  ...
  monaco.languages.setMonarchTokensProvider("customLang", {
    tokenizer: {
      ...
      archDiagram: [
        ...
        [
          new RegExp(
            `^(\\s*)(gap|layout|rotateRight|uses|connects|${annotationPropRegex})(?=\\s*:)`,
          ),
          ["", "arch-section"],
        ],
    ...
    for (const rawLine of lines) {
      ...
      if (insideBlock) {
        if (blockBracketDepth === 1) {
          ...
          if (/^\s*fontSize\s*:/.test(rawLine))
            usedBlockEntries.add("fontSize");
  ...
  function hasEdgeAnchorEndpointInArchitectureEdgeText(text) {
    return /\b[a-zA-Z_][a-zA-Z0-9_]*\.(start|mid|end)\b/.test(
      String(text || ""),
    );
  }
  ...
  function getArchitectureEdgeMemberCandidates(block, currentEdgeName = null) {
    return {
      ...
      edgeNames: (block?.edges || []).filter(
        (name) => name !== currentEdgeName,
      ),
    };
  }

  function getArchitectureEdgeCompletionContext(
    edgeText,
    block,
  ...
  function getDiagramConnectCompletionContext(connectText, arch) {
    ...
    const hasEdgeAnchorEndpoint =
      hasEdgeAnchorEndpointInDiagramConnectText(text);
  ...
  function getAllowedArrowheadValuesForShapeText(text) {
      ...
      try {
        ...
        function getArchitectureInlineNamespaceContext(model, position) {
          ...
          if (diagramCtx.insideDiagramConnects) {
            ...
            const ns = parseArchitectureNamespaceAccess(
              connectText.trimStart(),
            );
        ...
        if (inlineNamespaceCtx && !context.methodCallContext) {
          ...
          function getArchitecturePropertyInsertText(fullName) {
        ...
        function pushConnectionInlineProps(
          suggestions,
          monaco,
          range,
          usedProps,
        ...
        function pushNodeInlinePropertySuggestions(
          ...
          const isNamespaceFullyUsed = (namespace) => {
            const children = (namespaceChildren[namespace] || []).filter(
        ...
        function isCompletedColorArrayItemForNodeColor(linePrefix) {
          ...
          fshouldSuppressArchitectureInlineValueSuggestions;
